...
export function renameProperties<T extends object>(name: string, replace: string, obj: T): T {
  ...
  for (const key of Object.keys(obj)) {
    ...
    if (key.includes(capitalizedName)) {
      const capitalizedReplace = replace
        ...
        .map((c, i) => (i === 0 ? c.toUpperCase() : c))
        .join('')
      renamed[key.replace(capitalizedName, capitalizedReplace)] = obj[key as keyof T]
      continue
    }
